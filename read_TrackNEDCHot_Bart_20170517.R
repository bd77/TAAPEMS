...
write.table(overview.TrackHotNEDC.sorted, 'overview_TrackHotNEDC.txt', row.names = FALSE, quote = FALSE, sep = ';')
...
for (path in paths) {
  ...
  for (file in xlsx.files) {
    ...
    if (NROW(test.data) == 0) {
      ...
      if (header.names[3] == 'Absolute_time') {
      ...
      } else if (header.names[3] == 'Test.SequenceNumber') { # R replaces spaces by a point
        
        # There is a format with 36 and 52 columns. The extra columns contain OBD data but
        # they're empty or constant. So the are just left out
        ...
        # add a column with the car name and test type
        test.data <- data.frame(car.name = car.name, test.type = 'T-Hot NEDC', test.data)
        Track.hotNEDC.results.2 <- rbind(Track.hotNEDC.results.2, test.data[,1:36])
        
      } else if (header.names[3] == 'TestSequenceNumber') {
